...
def main():
    ...
    replaceStrInFile("./tmrg/toolset.py",'tmrg_version.str=""','tmrg_version.str="%s"'%version)
    ...
    cmd="""cd .. ; tar --exclude='activity.py' --exclude='fastLefParser.py' --exclude='top.py' --exclude='*.pyc' --exclude='*~' \
          --exclude='reportPrints.py' --exclude='sdf.py' --exclude='spliter.py' --exclude="rc" \
          --exclude="examples/doc"  --exclude="examples/*.py" --exclude="examples/*.cnf" \
          -cvzf  %s tmrg/doc/html tmrg/doc/pdf tmrg/tmrg tmrg/etc tmrg/common tmrg/bin tmrg/examples  tmrg/README.md tmrg/LICENSE.md && mv %s tmrg"""%(fname,fname)
#    print cmd
    os.system(cmd)
